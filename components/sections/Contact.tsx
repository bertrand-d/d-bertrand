...
function Info({
  ...
  const content = (
    <span className="font-display block text-lg text-white wrap-anywhere">
      {value}
    </span>
  );

  return (
    <li className="min-w-0 rounded-2xl border border-white/8 bg-white/4 px-5 py-4">
      <p className="text-xs uppercase tracking-widest text-grey">{label}</p>
      {href ? (
        <a href={href} className="mt-1 block min-w-0 hover:text-primary-bright">
          {content}
        </a>
      ) : (
        <p className="mt-1 min-w-0">{content}</p>
      )}
    </li>
  );
}
